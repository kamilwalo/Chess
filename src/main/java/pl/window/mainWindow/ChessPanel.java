...
public class ChessPanel extends JPanel implements MouseListener {
    ...
    private final Color FIRST_SQUARE_COLOR = Color.getHSBColor(131,158,99);
    private final Color SECOND_SQUARE_COLOR = Color.getHSBColor(235,236,208);
    ...
    private void drawChosePieceBackground(Graphics g) {
    ...
    private void drawPossibleMoves(Graphics g) {
        ...
        for (int Y = 0; Y < Board.tempPiece.getPossibleMoves().length; Y++) {
            for (int X = 0; X < Board.tempPiece.getPossibleMoves().length; X++) {
                if (Objects.equals(Board.tempPiece.getPossibleMoves()[X][Y], "Y") || Objects.equals(Board.tempPiece.getPossibleMoves()[X][Y], "E")){
                ...
                if (Objects.equals(Board.tempPiece.getPossibleMoves()[X][Y], "K")){
    ...
    private void drawPieces(Graphics g) {
        for (int i = 0; i < Board.black_pieces.length; i++) {
            if(Board.black_pieces[i].getStatus())
            ...
            if(Board.white_pieces[i].getStatus())
    ...
    private void drawChessBoard(Graphics g) {
        for(int i = 0; i<SQUARES; i++){
            for (int j = 0; j < SQUARES; j++) {
                if ((i+j)%2==0) g.setColor(SECOND_SQUARE_COLOR);
                else g.setColor(FIRST_SQUARE_COLOR);
    ...
    private void makeAction(MouseEvent e){
        Point pointClicked = new Point((e.getX() / (WINDOW_WIDTH / SQUARES)), (e.getY() / (WINDOW_HEIGHT / SQUARES)));
        try {
